...
DEFAULT_EXCEL_CANDIDATES = [
    str(TESTS_DIR / "Assignment_1_-_Test_cases_Negative.xlsx"),
...
def _last_data_row(ws, header_row: int, input_col_idx: int) -> int:
    last_row = header_row
...
def _count_statuses(ws, header_row: int, status_col_idx: int) -> dict:
    counts = {"PASS": 0, "FAIL": 0, "UI Error": 0, "COLLECTED": 0}
    for row_index in range(header_row + 1, int(ws.max_row or 0) + 1):
        status_value = _merged_top_left_cell(ws, row_index, status_col_idx).value
        status_text = str(status_value).strip() if status_value is not None else ""
        if status_text in counts:
            counts[status_text] += 1
    counts["TOTAL"] = counts["PASS"] + counts["FAIL"] + counts["UI Error"] + counts["COLLECTED"]
    return counts
...
def run_test():
    ...
    counts = _count_statuses(ws, header_row, status_col_idx)
    print(
        f"Final Summary: PASS={counts['PASS']}, FAIL={counts['FAIL']}, "
        f"UI Error={counts['UI Error']}, COLLECTED={counts['COLLECTED']}, TOTAL={counts['TOTAL']}"
    )
